...
self.addEventListener('fetch', e => {
    ...
    // State-changing calls (form POSTs, login, logout, API mutations) â€” NEVER intercept! Always direct to native browser network stack
    if (req.method !== 'GET') {
        return;
    }

    // Dynamic API requests (?api=...) â€” always live network, never cached by SW.
    ...
    // â”€â”€ IMAGES: cache-first (instant after first view, works offline) â”€â”€
    ...
    // â”€â”€ APP HTML NAVIGATION: Always live network (fresh user session) with clean offline screen fallback â”€â”€
    if (req.mode === 'navigate') {
        e.respondWith((async () => {
            try {
                // Direct live network request â€” ensures account switches, logins, and session state are 100% instant
...
<button class="btn btn-primary" onclick="location.reload()">ðŸ”„ Retry Connection</button>
<a href="${dashUrl}" class="btn btn-secondary">ðŸ  Back to Dashboard</a>
</div>
</body>
    ...
    // â”€â”€ CDN libraries: cache-first (immutable) â”€â”€
