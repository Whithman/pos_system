...
function Build-EscPosReceipt {
    ...
    # 2. Cash drawer kick pulse: ESC p 0 25 250
    ...
    if ($data.items) {
        foreach ($it in $data.items) {
            ...
            $priceStr = [string]::Format("{0:N2}", [double]$it.price)
            $totalStr = [string]::Format("{0:N2}", ([double]$it.qty * [double]$it.price))
    ...
    $subtotal = [string]::Format("{0}{1:N2}", $cur, [double]$data.subtotal)
    $vat = [string]::Format("{0}{1:N2}", $cur, [double]$data.vat)
    $tax = [string]::Format("{0}{1:N2}", $cur, [double]$data.tax)
    $total = [string]::Format("{0}{1:N2}", $cur, [double]$data.total)
    $cash = [string]::Format("{0}{1:N2}", $cur, [double]$data.cash)
    $change = [string]::Format("{0}{1:N2}", $cur, [double]$data.change)
    $itemCount = "$($data.item_count)"

    $bw.Write($enc.GetBytes((Format-ReceiptLine "SUBTOTAL:" $subtotal)))
    $bw.Write($enc.GetBytes((Format-ReceiptLine "VAT ($($data.vat_rate)%):" $vat)))
    $bw.Write($enc.GetBytes((Format-ReceiptLine "TAX ($($data.tax_rate)%):" $tax)))
